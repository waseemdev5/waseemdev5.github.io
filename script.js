...
(function () {
    ...
    courseForm.addEventListener('submit', function (e) {
        ...
        renderCourses();
        renderOrders();
        syncHomepage();  // ← This syncs changes to homepage
        courseForm.reset();
        imageInput.value = '';
    });

    // ---------- SYNC HOMEPAGE (Keep courses visible) ----------
    function syncHomepage() {
        // Always show courses on homepage
        var coursesSection = document.getElementById('courses');
        if (coursesSection) {
            coursesSection.style.display = '';
        }
        
        // Make sure all sections are visible on homepage
    ...
    loginForm.addEventListener('submit', function (e) {
        ...
        if (user === 'admin' && pass === 'hattar1234') {
            ...
            // Hide only navbar when in admin mode
